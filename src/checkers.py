...
class Board:
    ...
    def move_piece(self, old_pos, new_pos, team):
        ...
        if not self.can_move(old_pos) or not current_piece.can_move(new_pos):
            print("Invalid Move")

        if team == "Red":
            if abs((new_pos[1] - old_pos[1])) == 1:
                if new_pos[0] != 0:
                    new_piece = current_piece
                if new_pos[0] == 0:
                    new_piece = King((new_pos[0],new_pos[1]))
            ...
            if abs(new_pos[1] - old_pos[1]) == 2:
                ...
                if new_pos[0] == 0:
                    new_piece = King((new_pos[0],new_pos[1]))
                if new_pos[1] - old_pos[1] == 2:
                    self.black_pieces.remove(self.game_board[old_pos[0] + 1][old_pos[1] + 1])
                if new_pos[1] - old_pos[1] == -2:
                    self.black_pieces.remove(self.game_board[old_pos[0] + 1][old_pos[1] - 1])
    ...
    def can_capture(self,pos):
        ...
        if not piece.is_king():
            if piece.team == "Red":
                if pos[0] <= 1:
                    return False
                elif (pos[1] > 1 and not self.game_board[pos[0] - 1][pos[1] -1].is_empty() 
                    self.game_board[pos[0] - 1][pos[1] -1].team == "Black" and
                    self.game_board[pos[0] - 2][pos[1] -2].is_empty()):
                    return True
                elif (pos[1] < (self.width - 2) not self.game_board[pos[0] - 1][pos[1] + 1].is_empty() and
                    self.game_board[pos[0] - 1][pos[1] + 1].team == "Black" and
                    self.game_board[pos[0] - 2][pos[1] + 2].is_empty()):
                    return True
            if piece.team == "Black":
                if pos[0] >= self.width - 2:
                    return False
                elif (pos[1] < (self.width - 2) and 
                    not self.game_board[pos[0] + 1][pos[1] + 1].is_empty() and
                    self.game_board[pos[0] + 1][pos[1] + 1].team == "Red" and
                    self.game_board[pos[0] + 2][pos[1] + 2].is_empty()):
                    return True
                elif (pos[1] > 1 and not self.game_board[pos[0] - 1][pos[1] + 1].is_empty() and
                    self.game_board[pos[0] + 1][pos[1] - 1].team == "RED" and
                    self.game_board[pos[0] + 2][pos[1] - 2].is_empty()):
                    return True
            return False
        else:
            if pos[0] < self.width - 2 and pos[1] < self.width - 2:
                neighbor_1 = self.game_board[pos[0] + 1][pos[1] + 1] 
            if pos[0] < self.width - 2 and pos[1] > 1:
                neighbor_2 = self.game_board[pos[0] + 1][pos[1] - 1]
            if pos[0] > 1 and pos[1] > 1:
                neighbor_3 = self.game_board[pos[0] - 1][pos[1] - 1]
            if pos[0] > 1 and pos[1] < self.width - 2
                neighbor_4 = self.game_board[pos[0] - 1][pos[1] + 1]
            if neighbor_1 is not None:
                if (not neighbor_1.is_empty() and self.game_board[pos[0] + 2][pos[1] + 2].is_empty()
                    and self.team != neighbor_1.team):
                    return True
            if neighbor_2 is not None:
                if (not neighbor_2.is_empty() and self.game_board[pos[0] + 2][pos[1] - 2].is_empty()
                    and self.team != neighbor_2.team):
                    return True
            if neighbor_3 is not None:
                if (not neighbor_3.is_empty() and self.game_board[pos[0] - 2][pos[1] - 2].is_empty()
                    and self.team != neighbor_3.team):
                    return True
            if neighbor_4 is not None:
                if (not neighbor_4.is_empty() and self.game_board[pos[0] - 2][pos[1] -+2].is_empty()
                    and self.team != neighbor_4.team):
                    return True
            return False
    ...
    def can_move(self, pos):
        ...
        if self.can_capture(pos):
            return True
        if ((self.game_board[pos[0] + 1][pos[1] + 1].is_empty() and pos[1] <= self.width - 2 
            and pos[0] <= self.width - 1) or 
            (self.game_board[pos[0] + 1][pos[1] - 1].is_empty() and pos[1] >=1 
            and pos[0] <= self.width - 1)):
            if piece.team == "Black" or piece.is_king():
                return True
       if ((self.game_board[pos[0] - 1][pos[1] - 1].is_empty() and pos[1] >= 1 and 
            and pos[0] >= 0) or (self.game_board[pos[0] - 1][pos[1] + 1].is_empty()
            and pos[1] <= self.width - 2 and pos[0] >= 0))):
            if piece.team == "Red" or piece.is_king():
                return True
        return False
    ...
    def is_valid_move(self, curr_pos, new_pos):
        ...
        """
        piece = self.game_board[curr_pos[0]][curr_pos[1]]
        new_spot = self.game_board[new_pos[0]][new_pos[1]]
        if ((curr_pos[0] - new_pos[0]) == 1 and abs(curr_pos[1] - new_pos[1]) == 1 and 
            new_spot.is_empty()):
            if piece.team == "Black" or piece.is_king():
                return True
        if ((curr_pos[0] - new_pos[0]) == -1 and abs(curr_pos[1] - new_pos[1]) == 1 and 
            ...
            if piece.team == "Red" or piece.is_king():
                return True
        if ((curr_pos[0] - new_pos[0]) == 2 and (curr_pos[1] - new_pos[1]) == 2
            and new_spot.is_empty and not self.game_board[curr_pos[0] + 1][curr_pos[1] + 1].is_empty())
            if (piece.team == "Black" or piece.is_king() and 
                piece.team != self.game_board[curr_pos[0] + 1][curr_pos[1] + 1].team) :
                return True
        if ((curr_pos[0] - new_pos[0]) == 2 and (curr_pos[1] - new_pos[1]) == -2
            and new_spot.is_empty and not self.game_board[curr_pos[0] + 1][curr_pos[1] - 1].is_empty())
            if (piece.team == "Black" or piece.is_king() and 
        ...
        if ((curr_pos[0] - new_pos[0]) == -2 and (curr_pos[1] - new_pos[1]) == -2
            and new_spot.is_empty and not self.game_board[curr_pos[0] - 1][curr_pos[1] - 1].is_empty())
            if (piece.team == "Red" or piece.is_king() and 
                piece.team != self.game_board[curr_pos[0] - 1][curr_pos[1] - 1].team) :
                return True
        if ((curr_pos[0] - new_pos[0]) == 2 and (curr_pos[1] - new_pos[1]) == 2
            and new_spot.is_empty and not self.game_board[curr_pos[0] + 1][curr_pos[1] + 1].is_empty())
